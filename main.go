...
func ApiServer(ctx context.Context) error {
	...
	if err != nil {
		log.Fatal().Msgf("failed to generate access key pair: %v", err)
	}

	refreshPublicKey, refreshPrivateKey, err := ed25519.GenerateKey(nil)
	if err != nil {
		log.Fatal().Msgf("failed to generate refresh key pair: %v", err)
	}

	//start hacks module
	hackJwt := auth_jwt.NewJwt(accessPrivateKey, accessPublicKey, refreshPrivateKey, refreshPublicKey, "kodiiing", "user", "kodiiing")
	hackAuthService := authservice.NewAuthService(config.Environment, pgxPool, memory)
	hackAuthMiddleware := auth_middleware.NewAuthMiddleware(hackAuthService, hackJwt)
	hackProviderTypesense := hackprovider.NewHackTypesense(search)
	hackProviderSQL := hackprovider.NewHackYugabyte(pgxPool)
	hackService := hackservice.NewHackService(config.Environment, hackAuthMiddleware, *hackProviderSQL, *hackProviderTypesense)
	//end hacks module

	app := chi.NewRouter()

	app.Mount("/Hack", hackstub.NewHackServiceServer(hackService))
